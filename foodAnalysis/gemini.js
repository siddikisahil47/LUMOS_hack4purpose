...
export async function generateContent() {
    try {
        ...
        const jsonUrl = "https://twis.in/shop/test/img/image_data.json";
        ...
        const parts = [
            { text: "you are a helpful Health food assistant. You know that which food is healthy and which food is unhealthy. Your task is to tell me which food is healthy and which food is unhealthy according to that food image and also provide me the percentage of harmness and also calories of that food and my medical conditions are " + md + " now tell me will i eat or not also it is important to tell me the percentage of harmness and remember that response should be 100 word and do not use hello or hey or hi and do not use any greetings"  },
            {
                inlineData: {
                    mimeType: "image/png",
                    data: imageformat
...
export async function postData(text) {
    const response = await fetch('https://twis.in/shop/test/response.php', {
        method: 'POST',
        headers: {
